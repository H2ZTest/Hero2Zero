...
contract Voting is ReentrancyGuard, AccessControl {  
    ...
    uint public constant VOTING_DURATION = 300;  // 1 weeks
    uint public constant MINIMUM_QUORUM = 1;
    uint public constant ABSTAIN = 1;
    uint public constant YES = 2;
    uint public constant NO = 3;
    uint public constant NO_WITH_VETO = 4;
    
    string public constant reVoting = "That theme is expired and recreated";
    ...
    function createSession(string calldata _theme, string calldata _description, uint _quorum)external onlyRole(DEFAULT_ADMIN_ROLE) returns(bytes32 _sessionId){
    ...
    function voteFor(bytes32 _sessionId, uint _voteType)external {
        address _user = msg.sender;
        require(_voteType == ABSTAIN || _voteType == YES || _voteType == NO || _voteType == NO_WITH_VETO, "Voting: Choosed wrong vote type");
        ...
        require(sessions[_sessionId].votingEnd > block.timestamp, "Voting: That session is ended already");
        ...
        require(IERC721(SBToken).balanceOf(_user) > 0, "Voting: Its fiasco, brah");
    ...
    function confirmSession(bytes32 _sessionId)external returns(string memory result){
        ...
        uint _totalVotes = sessions[_sessionId].abstainVotes + sessions[_sessionId].yesVotes + sessions[_sessionId].noVotes + sessions[_sessionId].noWithVetoVotes;
    ...
    function calculateSessionId(uint _number, address _creator, bytes32 _theme, bytes32 _description, uint _votingStart, uint _quorum)public pure returns(bytes32){
