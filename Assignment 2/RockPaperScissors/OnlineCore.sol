...
contract OnlineCore is AccessControl{
    ...
    function createOnlineGameBNB(address _user, bytes32 _answerHash, uint _bet)external onlyRole(ROUTER_CALLER) returns(uint _gameId){
        onlineDefineData(address(0), _user, _answerHash, _bet);
        _gameId = totalOnlineBatches;
        totalOnlineBatches += 1;
    }

    function participateOnlineGameBNB(address _user, uint _gameId, uint _answer)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(uint bet, address user){
        require(onlineBatches[_gameId].closed == false, "OnlineCore: Game is closed already");
    ...
    function finishOnlineGameBNB(address _user, uint _gameId, uint _answer, string memory _salt)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(address user2, address winner, uint bet){
    ...
    function refundBetFromExpiredOnlineGameBNB(address _user, uint _gameId)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(address user1, uint bet){
    ...
    function cancelOnlineGameBNB(address _user, uint _gameId)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(uint bet){
        checkParticipateOne(_user, _gameId);
        checkClosed(_gameId);
    ...
    function createOnlineGameToken(address _user, bytes32 _answerHash, address _token, uint _amount)external onlyRole(ROUTER_CALLER) returns(uint _gameId){
    ...
    function participateOnlineGameToken(address _user, uint _gameId, uint _answer)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(uint bet, address user1, address token){
    ...
    function finishOnlineGameToken(address _user, uint _gameId, uint _answer, string memory _salt)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(address winner, address user2, address token, uint bet){
        checkParticipateOne(_user, _gameId);
        checkClosed(_gameId);
        require(onlineBatches[_gameId].playerTwo != address(0), "OnlineCore: Player 2 is not set yet");
        require(onlineBatches[_gameId].startTimeToProof + PROOF_DURATION >= block.timestamp, "OnlineCore: Expired");
    ...
    function refundBetFromExpiredOnlineGameToken(address _user, uint _gameId)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(address user1, address token, uint bet){
        checkParticipateTwo(_user, _gameId);
        checkClosed(_gameId);
        require(block.timestamp >= onlineBatches[_gameId].startTimeToProof + PROOF_DURATION, "OnlineCore: Not expired");
        onlineBatches[_gameId].winner = onlineBatches[_gameId].playerTwo;
    ...
    function cancelOnlineGameToken(address _user, uint _gameId)external onlineGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(address token, uint bet){
        checkParticipateOne(_user, _gameId);
        checkClosed(_gameId);
        require(onlineBatches[_gameId].playerTwo == address(0), "OnlineCore: You are have opponent already");
    ...
    function checkWinnerInternal(address user1, uint answerOne, address user2, uint answerTwo)internal pure returns(address winner){
