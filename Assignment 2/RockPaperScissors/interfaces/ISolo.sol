...
interface ISolo{
    ...
    function createSoloGameBNB(address user, uint answer, uint bet)external  returns(uint gameId);

    function finishSoloGameBNB(address user, uint gameId)external returns(address winner, uint bet);
