...
contract SoloCore is AccessControl{
    ...
    function createSoloGameBNB(address _user, uint _answer, uint _bet)external onlyRole(ROUTER_CALLER) returns(uint _gameId){ 
    ...
    function finishSoloGameBNB(address _user, uint _gameId)external soloGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(address winner, uint _bet){
        checkParticipateOne(_user, _gameId);
        checkClosed(_gameId);
        (bool fulfilled, uint256[] memory randomWords) = IOracle(oracle).getRequestStatus(soloBatches[_gameId].oracleRequestId);
        require(fulfilled == true, "SoloCore: Too soon");
        uint answerTwo = randomWords[0] % 3;
        soloBatches[_gameId].answerTwo = answerTwo;
        uint answerOne = soloBatches[_gameId].answerOne;
        _bet = soloBatches[_gameId].bet;
        winner = checkWinnerInternal(_user, answerOne, address(this), answerTwo);
        soloBatches[_gameId].winner = winner;
    }

    function createSoloGameToken(address _user, uint _answer, address _token, uint _amount)external onlyRole(ROUTER_CALLER) returns(uint _gameId){
    ...
    function finishSoloGameToken(address _user, uint _gameId)external soloGameExist(_gameId) onlyRole(ROUTER_CALLER) returns(address winner, address token, uint bet){
        checkParticipateOne(_user, _gameId);
        checkClosed(_gameId);
        (bool fulfilled, uint256[] memory randomWords) = IOracle(oracle).getRequestStatus(soloBatches[_gameId].oracleRequestId);
        require(fulfilled == true, "SoloCore: Too soon");
        ...
        winner = checkWinnerInternal(_user, answerOne, address(this), answerTwo);
        soloBatches[_gameId].winner = winner;     
        bet = soloBatches[_gameId].bet;
        token = soloBatches[_gameId].token;
    }
